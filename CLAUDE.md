...
| File | Purpose |
|---|---|
| STATE.md | Session handoff: where we are, atlas maps, pitfalls — read FIRST when resuming |
| AGENTS.md | Contract for the second agent (Codex, assets-only lane) — its CLAUDE.md |
| ASSET_SPEC.md | Atlas cell authority + drop-folder art pipeline; check its "Requests to Claude" section each session |

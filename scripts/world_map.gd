...
func _build_faction_row(id: String) -> HBoxContainer:
	...
	var widgets := {}
	for key in ["strength", "attitude", "status"]:
		var label := Label.new()
...
func _refresh() -> void:
	...
	for id: String in _rows:
		...
		w.strength.text = "str %d" % int(f.strength)
		w.attitude.text = "att %d" % int(f.attitude)
	...
	_expedition_label.text = "Expedition in the field — back at dawn." \
			if FactionManager.expedition_active() else "No expedition afield (needs 2+ armed villagers)."

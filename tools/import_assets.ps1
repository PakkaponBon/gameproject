...
#   tile_NN_name.png   -> assets/tiles.png   (cells 0..46)
#   sprite_NN_name.png -> assets/sprites.png (cells 0..71)
$repo = "C:\colony-sim"
$incoming = Join-Path $repo "assets\incoming"
if (-not (Test-Path $incoming)) { New-Item -ItemType Directory $incoming | Out-Null }

$targets = @{
    "tile" = @{ "path" = (Join-Path $repo "assets\tiles.png"); "max" = 46 }
    "sprite" = @{ "path" = (Join-Path $repo "assets\sprites.png"); "max" = 71 }
}
